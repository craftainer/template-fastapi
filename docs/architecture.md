...
Infrastructure/deployment view of the system: the dev service (`app`) and the
...
`.devcontainer/compose.yml` builds `app` from the `Dockerfile`'s
...
browser Playwright drives for the e2e suite). `app` declares a
`depends_on: <service>: condition: service_healthy` entry for each one,
so Compose won't start `app` until every dependency's own healthcheck
passes — see
...
```mermaid
graph LR
    app -->|depends_on: service_healthy| postgres[(Postgres)]
    app -->|depends_on: service_healthy| s3[(RustFS / S3)]
    app -->|depends_on: service_healthy| redis[(Redis)]
    app -->|depends_on: service_healthy| keycloak[Keycloak]
    app -->|depends_on: service_healthy| selenium[Selenium]
```
...
publishing it via Compose. `app`'s own `:8000` is forwarded the same
way, for the same reason.

| Service   | Container-internal port(s) | Host-forwarded?                     |
| --------- | --------------------------- | ------------------------------------ |
| `app`     | 8000                         | yes — `forwardPorts` (devcontainer)  |
| postgres  | 5432                         | no                                    |
...
`app` loads each stack service's own local-dev-only credentials via
